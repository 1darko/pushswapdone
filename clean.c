...
void set_a_on_top2(t_stack **a, t_stack *cheap)
{
...
void push_b_to_a(t_stack **a, t_stack **b, int size)
{
    ...
    // while ((*b)->nbr != cheap->nbr || (*a)->nbr != cheap->target->nbr)
    // {
    // if((*b)->above_median == 1 && (*b)->target->above_median == 1)
    //     rotate_up_b(a, b, (*b)->target);

    // if ((*b)->above_median == 0 && (*b)->target->above_median == 0)
